...
/**
 * Servlet implementation class FleaBuyListController
 */
@WebServlet("/buyBook.fl")
public class FleaBuyListController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public FleaBuyListController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // *listCount : 총 게시글 갯수
        listCount = new FleaMarketService().selectBuyListCount();
		...
		ArrayList<FleaMarket> list = new FleaMarketService().selectFleaMarketBuyList(pi);
		
		request.getSession().setAttribute("list", list);
		request.getSession().setAttribute("pi", pi);
		
		request.getRequestDispatcher("views/fleaMarket/fleaBuyListView.jsp").forward(request, response);
	}
