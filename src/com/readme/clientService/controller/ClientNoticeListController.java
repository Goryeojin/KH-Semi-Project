...
/**
 * Servlet implementation class ClientNoticeListController
 */
@WebServlet("/noticeList.cs")
public class ClientNoticeListController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ClientNoticeListController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int maxPage; // 가장 마지막페이지가 몇번 페이지인지 (== 총페이지의 갯수)
	 	...
	 	// *listCount : 총 게시글 갯수
	 	listCount = new ClientService().selectListCount();
		...
		ArrayList<Notice> list = new ClientService().selectNoticeAll(pi);
		
		request.setAttribute("list", list);
		request.setAttribute("pi", pi);
		request.getRequestDispatcher("views/clientService/clientNoticeListView.jsp").forward(request, response);
	}
