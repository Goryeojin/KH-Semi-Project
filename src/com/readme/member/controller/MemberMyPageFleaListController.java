...
/**
 * Servlet implementation class MemberMyPageFleaListController
 */
@WebServlet("/fleaList.me")
public class MemberMyPageFleaListController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public MemberMyPageFleaListController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getSession().getAttribute("loginUser") != null) {
			...
			// *listCount : 총 게시글 갯수
			listCount = new MemberService().selectFleaCount(userNo);
			...
			ArrayList list = new MemberService().findAllFleaMarket(userNo, pi);
			
			request.setAttribute("fleaMarketList", list);
			request.setAttribute("pi", pi);
		}
		request.getRequestDispatcher("views/member/myPageFleaListView.jsp").forward(request, response);
	}
