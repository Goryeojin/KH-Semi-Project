...
public class MemberMyPageReviewListController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public MemberMyPageReviewListController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getSession().getAttribute("loginUser") != null) {
			...
			// *listCount : 총 게시글 갯수
			listCount = new MemberService().selectReviewCount(userNo);
			...
			ArrayList list = new MemberService().findAllReviews(userNo, pi);
			
			request.setAttribute("reviews", list);
			request.setAttribute("pi", pi);
		}
		
		request.getRequestDispatcher("views/member/myPageReviewListView.jsp").forward(request, response);
	}
