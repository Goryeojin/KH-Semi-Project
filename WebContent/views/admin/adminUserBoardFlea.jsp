<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import = "java.util.ArrayList, com.readme.admin.model.vo.PageInfo, com.readme.admin.model.vo.FleaAd" %>
<%
	ArrayList<FleaAd> list = (ArrayList<FleaAd>)request.getAttribute("list");
...
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.6.1/dist/css/bootstrap.min.css">
    ...
    <title>관리자페이지 - 회원 글 관리(중고장터)</title>
    <style>
   body { margin: 0;}
    div{
...
<body>
    ...
    <div id="admin_content">
		...
		<form action="<%= contextPath %>/deleteFlea.ad" method="post">
        ...
        <div id="mainTable" >
            <table border="1">
                <tr>
                    <th width="30"><input type="checkbox" name="all" class="batch" id="all"></th>
                    <th width="60">번호</th>
                    <th width="150">아이디</th>
			...
			<% if(list.isEmpty()){ %>
             ...
             <%}else{ %>
             	<% for(FleaAd f : list){ %>       
                <tr>
                    <td><input type="checkbox" class="batch" name="checkList" value="<%=f.getFleaNo() %>"></td>
                    <td><%=f.getFleaNo() %></td>
                    <td><%=f.getUserId() %></td>
                    <td><%=f.getFleaDate() %></td>
                    <td><%=f.getFleaTitle() %></td>
                    <td><%=f.getFleaType() %></td>
                    <td><%=f.getFleaStatus() %></td>
                </tr>
                <%} %>
               <%} %>
            </table>
        ...
        <div class="paging-area">
           <!--  페이징 버튼 -->
        	 ...
        	 <% if(currentPage != 1){ %>
      			 ...
      			 <button onclick="location.href='<%= contextPath %>/userBoardFlea.ad?currentPage=<%= currentPage-1 %>'">&lt;</button>
       			<%} %>
        		
	        		<% for(int i = startPage; i <= endPage; i++){ %>
	        			<% if(i != currentPage){ %>
	        			<!-- http://localhost:8000/jsp/requetList.ad?currentPage=XX -->
	            		<button onclick="location.href='<%=contextPath%>/userBoardFlea.ad?currentPage=<%=i%>'"><%=i %></button>
