        ...
        <div id="contentAll">
            ...
            <div id="aCategory"> 
             <form action="<%=contextPath %>/categoryBookList.ad?currentPage=1" method="post">
                <select name="bookCategory" class="select-css" id="bookCategory" onchange="formChange(this.form)">
                   <%for(CategoryAd c : category){ %>
                    <option value="<%=c.getCategoryNo() %>" onclick="allList();"><%=c.getCategoryName() %></option> 
                   <%} %>
                </select> 
             </form>
            </div>
    		...
    		<% if(currentPage != 1){ %>
      			 ...
      			 <button onclick="location.href='<%= contextPath %>/categoryBookList.ad?currentPage=<%= currentPage-1 %>'">&lt;</button>
       			<%} %>
        		
	        		<% for(int i = startPage; i <= endPage; i++){ %>
	        			<% if(i != currentPage){ %>
	        			
	            		<button onclick="location.href='<%=contextPath%>/categoryBookList.ad?currentPage=<%=i%>'"><%=i %></button>
    ...
    <script>
		...
		function formChange(obj){
		 ...
		 function test(){
			 ...
			 location.href="<%=contextPath%>/bookList.ad?currentPage=1";
